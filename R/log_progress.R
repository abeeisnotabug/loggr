...
log_progress <- function(..., loggr_object, expr) {
  ...
  logg_file_names <- make_logg_file_names(loggr_object)

  count <- !loggr_object$count_explicitly

  logg_this_condition <- function(c) {
    logg_condition(c = c, get_time(), loggr_object$parent_id, worker_id, it_count, raw_list, raw_values, logg_file_names$err)
  }

  logg_iteration("start", start_time, loggr_object$parent_id, worker_id, it_count, raw_list, raw_values, logg_file_names$out)
  ...
  logg_iteration("end", start_time, loggr_object$parent_id, worker_id, it_count, raw_list, raw_values, logg_file_names$out)
