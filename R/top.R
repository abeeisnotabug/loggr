top <- function() {
  ...
  procs_mat <- stringr::str_split_fixed(stringr::str_trim(top_procs[2:length(top_procs)]), "[ ]+", 12)
  colnames(procs_mat) <- stringr::str_split_fixed(stringr::str_trim(top_procs[1]), "[ ]+", 12)
  ...
  mem_mat <- stringr::str_split_fixed(top_head[4:5], "[ (?!:)]+", 10)
  mem_mat[, c(3, 5, 7, 9)] <- stringr::str_replace_all(mem_mat[, c(3, 5, 7, 9)], ",", ".")
  mem_mat[, c(4, 6, 8)] <- stringr::str_replace_all(mem_mat[, c(4, 6, 8)], "[,.]", "")
